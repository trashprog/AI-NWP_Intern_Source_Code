...
def process_file(truth_file):
    ...
    for lead in lead_times:
        ...
        fc_ds = xr.open_dataset(
            os.path.join(forecast_path, fc_file_name),
            # engine='cfgrib',
            # backend_kwargs={"filter_by_keys": {"typeOfLevel":"isobaricInhPa", "shortName":["u","v","t","z","q"]}, "indexpath": ""}
        )
...
with ProcessPoolExecutor(max_workers=16) as executor:
    results = list(tqdm(executor.map(process_file, truth_files), total=len(truth_files), desc="Processing files"))
...
# --- Save RMSE dictionary as NPZ ---
npz_file = os.path.join(output_dir, f"{file_prefix}rmse_dict.npz")
np.savez(npz_file, **all_rmse)
print(f"Saved RMSE dictionary: {npz_file}")
...
for var in all_rmse:
    ...
    plt.ylabel("RMSE")
    plt.title(f"ERA5 vs Aurora RMSE: {var} (SEA Region)")
    plt.grid(True)
    plot_file = os.path.join(output_dir, f"{file_prefix}{var}_rmse.png")
