...
def run_season(month_set, label):
    ...
    # Save
    npz_path = os.path.join(output_dir, f"era5_auro_surface_energy_rmse_{label}.npz")
    ...
    plt.ylabel("RMSE")
    plt.title(f"Surface Temperature Energy RMSE ({label})")
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, f"era5_auro_surface_energy_rmse_{label}.png"), dpi=150)
    plt.close()
