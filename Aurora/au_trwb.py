...
def compute_rmse_for_file(truth_file, forecast_path):
    ...
    for lead in lead_times:
        ...
        fc_file_name = f"aurora_forecast_{fc_date_str}_{fc_hour_str}-out-{lead}.nc"
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures), desc="ERA5–Aurora RMSE"):
            result = fut.result()
            if result is None:
                continue
            _, data = result
    ...
    # Save results
    npz_path = os.path.join(output_dir, "era5_vs_auro_rmse_allvars.npz")
    ...
    for var, title in [("wind", "850 hPa Wind Vector"), 
        ...
        plt.title(f"ERA5 vs Aurora RMSE: {title} (SEA Region)")
        plt.xlabel("Lead Time [hours]")
        plt.ylabel("RMSE")
        plt.grid(True)
        plt.legend()
        plt.savefig(os.path.join(output_dir, f"era5_vs_auro_rmse_{var}.png"), dpi=150, bbox_inches="tight")
        plt.close()
