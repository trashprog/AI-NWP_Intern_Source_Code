...
def extract_month_from_filename(filename):
    m = re.match(r"era5_pl_(\d{4})-(\d{2})-(\d{2})_(\d{2})\.grib", filename)
    if not m:
        return None
    return int(m.group(2))   # month as integer
...
def process_file(truth_file):
    ...
    for lead in lead_times:
        ...
        if not (np.array_equal(truth_ds['latitude'].values, fc_ds['latitude'].values) and
            ...
            print('coords not aligned!')
            continue

        # Loop over levels for MSE/vorticity
        for level in PRESSURE_LEVELS_MSE: # [850, 200]
            ...
            fc_u = fc_ds['u'].sel(isobaricInhPa=level).squeeze().values
            fc_v = fc_ds['v'].sel(isobaricInhPa=level).squeeze().values
            fc_t = fc_ds['t'].sel(isobaricInhPa=level).squeeze().values
            fc_z = fc_ds['z'].sel(isobaricInhPa=level).squeeze().values
            fc_q = fc_ds['q'].sel(isobaricInhPa=level).squeeze().values
...
def run_season(months, label):
    print(f"\n=== Running season: {label} ===")
    
    # Filter truth files for the season
    season_truth = []
    for f in truth_files:
        month = extract_month_from_filename(f)
        if month in months:
            season_truth.append(f)

    print(f"Files selected for {label}: {len(season_truth)}")
    ...
    with ProcessPoolExecutor(max_workers=16) as executor:
        results = list(tqdm(
            executor.map(process_file, season_truth),
            total=len(season_truth),
    ...
    for res in results:
        for var in all_rmse:
            for lead in lead_times:
                all_rmse[var][lead].extend(res[var][lead])

    # Save NPZ
    npz_file = os.path.join(output_dir, f"{file_prefix}rmse_dict_{label}.npz")
    np.savez(npz_file, **all_rmse)
    print(f"Saved: {npz_file}")

    # Plots
    for var in all_rmse:
        plt.figure(figsize=(10,6))
        mean_rmse = [np.nanmean(all_rmse[var][lead]) if all_rmse[var][lead] else np.nan for lead in lead_times]
        plt.plot(lead_times, mean_rmse, marker='o')
        plt.xlabel("Lead time (h)")
        plt.ylabel("RMSE")
        plt.title(f"ERA5 vs Aurora RMSE: {var} ({label})")
        plt.grid(True)

        out = os.path.join(output_dir, f"{file_prefix}{var}_rmse_{label}.png")
...
if __name__ == "__main__":
    ...
    # NE monsoon: Dec, Jan, Feb, Mar
    run_season({12, 1, 2, 3}, "NE")

    # SW monsoon: Jun, Jul, Aug, Sep
